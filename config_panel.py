...
def launch_config_panel(run_game_callback, args=None):
    ...
    def choose_file(entry_var, mode="save"):
        path = filedialog.asksaveasfilename() if mode == "save" else filedialog.askopenfilename()
        if path:
            entry_var.set(path)
    ...
    def add_row(label, widget, row):
        ttk.Label(main_frame, text=label).grid(row=row, column=0, sticky="e", pady=5, padx=10)
        widget.grid(row=row, column=1, sticky="w", pady=5, padx=10)

    row = 0
    add_row("Training sessions:", ttk.Entry(main_frame, textvariable=sessions_var, width=10), row); row += 1

    visual_combo = ttk.Combobox(main_frame, textvariable=visual_var, values=["on", "off"], width=7)
    add_row("Visual mode:", visual_combo, row); row += 1

    save_frame = ttk.Frame(main_frame)
    ttk.Entry(save_frame, textvariable=save_var, width=30).pack(side="left")
    ttk.Button(save_frame, text="Browse", command=lambda: choose_file(save_var, "save")).pack(side="left", padx=5)
    add_row("Save model to:", save_frame, row); row += 1

    load_frame = ttk.Frame(main_frame)
    ttk.Entry(load_frame, textvariable=load_var, width=30).pack(side="left")
    ttk.Button(load_frame, text="Browse", command=lambda: choose_file(load_var, "load")).pack(side="left", padx=5)
    add_row("Load model from:", load_frame, row); row += 1

    add_row("Board size (10–42):", ttk.Entry(main_frame, textvariable=boardsize_var, width=10), row); row += 1
    add_row("Game speed:", ttk.Entry(main_frame, textvariable=speed_var, width=10), row); row += 1
    ...
    ttk.Button(main_frame, text="Start Human Snake Game", command=start_human_game).grid(row=row, column=0, columnspan=2, pady=10)
